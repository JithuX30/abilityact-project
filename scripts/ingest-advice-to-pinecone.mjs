/**
 * One-time ingest script: uploads AbilityAction advice content to Pinecone.
 * Run with: npm run ingest-advice
 * Safe to re-run — upsert with consistent IDs overwrites existing records.
 */

import path from "path";
import { fileURLToPath } from "url";
import { config } from "dotenv";
import { Pinecone } from "@pinecone-database/pinecone";

// ESM shim for __dirname
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Load keys from .env.local (never hardcode these)
config({ path: path.resolve(__dirname, "../.env.local") });

const { PINECONE_API_KEY, PINECONE_INDEX_HOST } = process.env;
if (!PINECONE_API_KEY || !PINECONE_INDEX_HOST) {
  console.error("Missing PINECONE_API_KEY or PINECONE_INDEX_HOST in .env.local");
  process.exit(1);
}

// ── Advice content ─────────────────────────────────────────────────────────
// Mirrored from src/lib/advice-data.js. We inline it here because the app
// file uses ESM `export` syntax but the project has no "type":"module", so
// Node.js can't import it directly from a .mjs script.
const TOPIC_DATA = {
  "employment-support": {
    slug: "employment-support",
    name: "Employment support",
    description:
      "Advice on finding work, workplace adjustments, and your rights as a disabled employee.",
    intro:
      "Finding and keeping work when you have a disability or health condition can come with extra challenges. Whether you're entering the job market, returning after ill health, or dealing with difficulties in your current role, our guides can help.",
    articles: [
      {
        slug: "finding-work",
        title: "Finding work as a disabled person",
        summary:
          "Practical steps to searching for jobs, writing applications, and preparing for interviews.",
      },
      {
        slug: "workplace-adjustments",
        title: "Reasonable adjustments at work",
        summary:
          "What adjustments employers are legally required to make, and how to ask for them.",
      },
      {
        slug: "access-to-work",
        title: "Access to Work: what it is and how to claim",
        summary:
          "The government grant that can help cover the extra costs of working with a disability.",
      },
      {
        slug: "disability-confident-employer",
        title: "What 'Disability Confident' means for you",
        summary:
          "How to spot employers who have committed to inclusive hiring practices.",
      },
    ],
  },

  "sign-language-services": {
    slug: "sign-language-services",
    name: "Sign language services",
    description:
      "BSL and SLSL interpreting, community classes, and deaf awareness resources.",
    intro:
      "AbilityAction offers British Sign Language (BSL) and Sri Lankan Sign Language (SLSL) interpreting, community classes, and deaf awareness training. Whether you are deaf, hard of hearing, or a hearing person wanting to connect, we have something for you.",
    articles: [
      {
        slug: "what-is-bsl",
        title: "What is British Sign Language (BSL)?",
        summary: "An introduction to BSL, the visual language of the UK Deaf community.",
      },
      {
        slug: "slsl-services",
        title: "Sri Lankan Sign Language (SLSL) support",
        summary: "Our unique SLSL interpreting and community support services.",
      },
      {
        slug: "booking-an-interpreter",
        title: "Booking a BSL or SLSL interpreter",
        summary:
          "How to arrange an interpreter for appointments, interviews, or everyday situations.",
      },
      {
        slug: "deaf-awareness-training",
        title: "Deaf awareness training for workplaces",
        summary:
          "How employers and colleagues can better support deaf and hard of hearing staff.",
      },
    ],
  },

  "housing-support": {
    slug: "housing-support",
    name: "Housing support",
    description:
      "Guidance on accessible housing, home adaptations, grants, and tenant rights.",
    intro:
      "Where you live has a huge impact on your independence and wellbeing. Our housing guides help disabled people find accessible homes, fund adaptations, and understand their rights as tenants and homeowners.",
    articles: [
      {
        slug: "finding-accessible-housing",
        title: "Finding accessible and adapted housing",
        summary:
          "How to search for accessible properties through local authorities and housing associations.",
      },
      {
        slug: "home-adaptations-grants",
        title: "Home adaptations and Disabled Facilities Grants",
        summary: "How to apply for a grant to adapt your home to meet your disability needs.",
      },
      {
        slug: "tenant-rights",
        title: "Your rights as a disabled tenant",
        summary:
          "What landlords must and cannot do under the Equality Act and housing law.",
      },
    ],
  },

  "mental-health-and-wellbeing": {
    slug: "mental-health-and-wellbeing",
    name: "Mental health and wellbeing",
    description:
      "Counselling, peer support, and wellbeing resources for disabled people.",
    intro:
      "Living with a disability can affect your mental health, and mental health conditions are themselves recognised as disabilities. Our guides help you access the right support, without stigma.",
    articles: [
      {
        slug: "accessing-counselling",
        title: "Accessing counselling and mental health support",
        summary: "How to find a counsellor or therapist, including free and low-cost options.",
      },
      {
        slug: "peer-support",
        title: "Peer support: connecting with others who understand",
        summary: "How peer support groups can help, and how to find ones near you.",
      },
      {
        slug: "mental-health-at-work",
        title: "Managing your mental health at work",
        summary:
          "Practical strategies and your legal rights when mental health affects your ability to work.",
      },
    ],
  },

  "benefits-and-entitlements": {
    slug: "benefits-and-entitlements",
    name: "Benefits and entitlements",
    description:
      "Clear guides on PIP, Universal Credit, ESA, and the benefits you may be owed.",
    intro:
      "Navigating the benefits system can be overwhelming. Our guides break it down clearly — helping you understand what you are entitled to, how to apply, and what to do if you are refused.",
    articles: [
      {
        slug: "pip-guide",
        title: "Personal Independence Payment (PIP): a complete guide",
        summary:
          "Who qualifies for PIP, how to apply, and what to do if your claim is refused.",
      },
      {
        slug: "universal-credit-and-disability",
        title: "Universal Credit and disability: what you need to know",
        summary:
          "How Universal Credit works for disabled people and those with health conditions.",
      },
      {
        slug: "esa-guide",
        title: "Employment and Support Allowance (ESA) explained",
        summary:
          "Understanding contribution-based and income-related ESA, and how to claim.",
      },
      {
        slug: "disability-premiums",
        title: "Disability premiums and benefit top-ups",
        summary:
          "Extra amounts you may be able to add to your existing benefits if you have a disability.",
      },
    ],
  },

  "your-rights": {
    slug: "your-rights",
    name: "Your rights",
    description:
      "Understanding the Equality Act 2010, discrimination law, and how to take action.",
    intro:
      "The law protects disabled people from discrimination in work, housing, education, and services. Our guides explain what those protections mean in practice and what to do if your rights are violated.",
    articles: [
      {
        slug: "equality-act-explained",
        title: "The Equality Act 2010: what it means for you",
        summary: "A plain-English guide to your legal protections as a disabled person.",
      },
      {
        slug: "disability-discrimination",
        title: "Types of disability discrimination and what to do",
        summary:
          "How to identify direct, indirect, and other forms of disability discrimination.",
      },
      {
        slug: "making-a-complaint",
        title: "How to make a formal complaint or claim",
        summary:
          "Step-by-step guidance on raising complaints, using employment tribunals, or contacting the EHRC.",
      },
    ],
  },
};

// ── Build chunks ───────────────────────────────────────────────────────────
// Each topic gets one intro chunk, each article gets one summary chunk.
// Text is written as natural prose so the embedding captures full context.
const records = [];

for (const [topicSlug, topic] of Object.entries(TOPIC_DATA)) {
  // Topic intro chunk — captures what this section is about
  records.push({
    id: `${topicSlug}-intro`,
    text: `${topic.name}: ${topic.description}\n\n${topic.intro}`,
    topic: topic.name,
    topicSlug,
    articleTitle: "",
    articleSlug: "",
    url: `/advice-and-support/${topicSlug}`,
    chunkType: "topic-intro",
  });

  // Per-article summary chunks
  for (const article of topic.articles) {
    records.push({
      id: `${topicSlug}-${article.slug}-0`,
      text: `${topic.name} — ${article.title}\n\n${article.summary}`,
      topic: topic.name,
      topicSlug,
      articleTitle: article.title,
      articleSlug: article.slug,
      url: `/advice-and-support/${topicSlug}/${article.slug}`,
      chunkType: "article-summary",
    });
  }
}

// ── Connect to Pinecone ────────────────────────────────────────────────────
const pc = new Pinecone({ apiKey: PINECONE_API_KEY });
// Pass the host directly so we don't need an extra describe-index API call
const index = pc.index("abilityact-advice", PINECONE_INDEX_HOST);

// ── Upsert in batches ──────────────────────────────────────────────────────
// Pinecone recommends ≤96 records per upsertRecords call (integrated embedding)
const BATCH_SIZE = 96;
console.log(`Preparing to upsert ${records.length} records in batches of ${BATCH_SIZE}...\n`);

let totalUpserted = 0;
for (let i = 0; i < records.length; i += BATCH_SIZE) {
  const batch = records.slice(i, i + BATCH_SIZE);
  const batchNum = Math.floor(i / BATCH_SIZE) + 1;
  process.stdout.write(`  Batch ${batchNum}: uploading ${batch.length} records... `);
  await index.namespace("").upsertRecords({ records: batch });
  totalUpserted += batch.length;
  console.log("done");
}

console.log(`\nUpsert complete. Total records uploaded: ${totalUpserted}`);

// ── Verify ─────────────────────────────────────────────────────────────────
// Stats can take a few seconds to reflect new records
console.log("\nQuerying index stats (may take a moment to update)...");
await new Promise((r) => setTimeout(r, 3000));
const stats = await index.describeIndexStats();
const count = stats.totalRecordCount ?? stats.namespaces?.[""]?.recordCount ?? "unknown";
console.log(`Index record count: ${count}`);
console.log("\nAll done. Run this script again any time — it will safely overwrite records by ID.");
